...
import logoLight from 'assets/banner-logo.png';
  ...
  return (
    <Box as="footer" sx={styles.footer}>
      <Container sx={styles.container}>
        ...
        >
          <Logo image={logoLight} sx={{width: '50px'}} />
          <Text
            as="p"
            sx={{
              color: '#ffffff',
          ...
          >
            Copyright by {new Date().getFullYear()} Overall.Cloud
          </Text>
        </Box>
        <Box sx={styles.linksWrap}>
          <Link path="/">Home</Link>
          <Link path="#recursos">Recursos</Link>
          <Link path="#preco">Preço</Link>
          <Link path="#depoimentos">Depoimentos</Link>
